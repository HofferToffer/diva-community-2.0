...
function MonthFeelingsTile({ profileId }: { profileId: string | undefined }) {
  ...
  return <HomeStatTile icon={HeartPulse} label={t("home.statFeelingsThisMonth")} value={`${monthDays} / ${daysElapsed}`} i={2} />;
}
  ...
  return (
    <div className="space-y-8">
      <motion.section {...fadeUp(0)} className="relative px-1 py-2">
        ...
        <p className="mt-1 font-display text-lg tracking-wide text-muted-foreground">{t("home.tagline")}</p>
      </motion.section>
      ...
      <section className="grid grid-cols-3 gap-3">
        <HomeStatTile icon={Route} label={t("home.statThisMonth")} value={formatKm(stats?.month_km ?? 0)} i={2} />
        <HomeStatTile
          icon={Activity}
          label={
            i18n.language === "en"
              ? t("home.statActivities", { count: (stats?.month_runs ?? 0) + (stats?.month_workouts ?? 0) })
      ...
      <motion.section {...fadeUp(3)}>
        <Button asChild size="lg" className="w-full">
          <Link to="/community/pridat/run">{t("nav.addActivity")}</Link>
        </Button>
      </motion.section>
      ...
      {challenge && (
        <motion.section {...fadeUp(5)} className="relative overflow-hidden rounded-2xl border border-border/50 bg-card p-5 shadow-elevated-sm">
          ...
          <Button asChild variant="link" className="mt-2 px-0">
            <Link to={`/community/challenges/${challenge.id}`}>{t("home.viewChallenge")}</Link>
          </Button>
        </motion.section>
      )}

      <motion.section {...fadeUp(6)} className="space-y-4">
        <h2 className="font-display text-2xl">{t("home.feed")}</h2>
        {feed.isLoading && (
